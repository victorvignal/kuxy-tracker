    ...
    <div className="flex-1 overflow-y-auto bg-bg">
      ...
      <div className="flex items-center gap-[14px] px-7 pt-6">
        <div className="flex gap-0.5 bg-bg-card border border-border rounded-[11px] p-1">
          {['contacts.tab_30d', 'contacts.tab_3m', 'contacts.tab_1y'].map((key) => (
            <button
              key={key}
              className="px-4 py-1.5 rounded-md text-[13px] font-medium text-text-muted hover:text-text hover:bg-bg-hover transition-colors"
            >
              {t(key)}
            </button>
          ))}
      ...
      <div className="px-7 pt-5 pb-10">
        <div className="bg-bg-card border border-border rounded-2xl shadow-card overflow-hidden">
          <div className="px-[22px] pt-5 pb-3.5 text-base font-bold tracking-tight">{title}</div>
          {/* Header row */}
          <div
            className="grid items-center px-[22px] pb-2.5 border-b border-border text-xs text-text-muted font-medium"
          ...
          {contacts.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="grid items-center px-[22px] py-[13px] border-b border-border text-[13.5px] hover:bg-bg-hover transition-colors"
                style={{ gridTemplateColumns: '44px 56px 1.5fr 1.9fr 110px 120px 90px' }}
              >
                <input
                  type="checkbox"
